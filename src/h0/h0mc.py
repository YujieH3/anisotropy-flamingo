...
# srun does not allocate the correct number of cores. mpiexec by itself does not
# get cores from the cluster.
...
def log_likelihood(params, X, Y, z_obs, phi, theta, yname, xname):
    ...
    # Anglular separation
    angle = cf.angular_separation(phi, theta, vlon, vlat) * np.pi / 180
...
def log_prior(params):
    ...
    delta, vlon, vlat, logA, B, sigma = params  # 6 parameters
...
if __name__ == '__main__':
    ...
    # default output: script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    OUTPUT = os.path.join(script_dir, "output.csv")
    ...
    parser.add_argument("output", type=str, nargs='?', help="Output file (default: script directory)", default=OUTPUT)
    ...
    Y = np.array(data[cf.COLUMNS_MC[yname]])
    X = np.array(data[cf.COLUMNS_MC[xname]])
    # Also load the position data
    phi = np.array(data["phi_on_lc"])
    theta = np.array(data["theta_on_lc"])
    # the observed redshift from lightcone
    z_obs = np.array(data["ObservedRedshift"])


    # Initial guess
    initial = np.array([0.1, 0, 0, 0.5, 1, 0.1])
    ...
    if SAMPLER == 'zeus':
        ...
        with ChainManager(nchains) as cm:
            ...
            sampler = zeus.EnsembleSampler(nwalkers, 
                                        ndim, 
                                        log_likelihood, 
                                        args=(X, Y, z_obs, phi, theta, yname, xname), 
                                        pool=cm.get_pool)
