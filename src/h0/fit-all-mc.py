...
if __name__ == '__main__':
    ...
    for ScalingRelation in cf.CONST.keys():
        ...
        Nclusters = cf.CONST_MC[ScalingRelation]['N'] # MC number of clusters (N=400)

        _ = ScalingRelation.find('-')
        Y = ClusterData[cf.COLUMNS_MC[ScalingRelation[:_  ]]][:Nclusters].values  # MC constants, T Chandra
        X = ClusterData[cf.COLUMNS_MC[ScalingRelation[_+1:]]][:Nclusters].values
        ...
        BestFitOutputFile = os.path.join(OutputFileDir, 'fit_all_mc.csv')
