...
#SBATCH -J h0mc_scatter_random     #Give it something meaningful.
...
# config
n=4       #multithreading doesn't pay off much
...
do
    ...
    # Run
    output="${analyse_dir}/lc${lc}/h0_mcmc_scatter_random.csv"
    flag="${analyse_dir}/lc${lc}/h0mc_scatter_random.done"

    if ! [ -f $flag ] #use a file flag
    then
        python /cosma/home/do012/dc-he4/anisotropy-flamingo/src/h0_anisotropy_mcmc/app2_h0mc_scatter_random_selection.py -i $input -o $output -n $n --overwrite && echo > $flag
    else
        echo "h0mc_scatter_random already done, skipping..."
    fi
done
