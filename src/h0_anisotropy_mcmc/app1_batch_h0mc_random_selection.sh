...
#SBATCH -J h0mc_random     #Give it something meaningful.
...
do
    ...
    if ! [ -f "${output}/h0mc_random.done" ] #use a file flag
    then
        python /cosma/home/do012/dc-he4/anisotropy-flamingo/src/h0_anisotropy_mcmc/app1_h0mc_random_selection.py -i $input -o "${output}/h0_mcmc_random.csv" -n $n --overwrite && echo > "${output}/h0mc_random.done"
    else
        echo "h0mc_random already done, skipping..."
    fi
done
