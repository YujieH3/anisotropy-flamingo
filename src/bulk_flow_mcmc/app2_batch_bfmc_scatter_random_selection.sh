...
#SBATCH -J bfmc_scatter_random     #Give it something meaningful.
...
do
    ...
    # Run
    output="${analyse_dir}/lc${lc}/bf_mcmc_scatter_random.csv"
    flag="${analyse_dir}/lc${lc}/bfmc_scatter_random.done"

    if ! [ -f $flag ] #use a file flag
    then
        python /cosma/home/do012/dc-he4/anisotropy-flamingo/src/bulk_flow_mcmc/app2_bfmc_scatter_random_selection.py -i $input -o $output -n -$n --overwrite && echo > $flag
    else
        echo "bfmc_scatter_random already done, skipping..."
    fi
done
