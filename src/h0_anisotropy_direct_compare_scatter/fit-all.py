...
if __name__ == '__main__':
    ...
    for ScalingRelation in cf.CONST.keys():
        ...
        Y = ClusterData[cf.COLUMNS[yname]][:Nclusters].values
        X = ClusterData[cf.COLUMNS[xname]][:Nclusters].values
        ...
        # Load error
        eY = ClusterData['e'+cf.COLUMNS[yname]][:Nclusters].values   # in ratio 0-1
        eX = ClusterData['e'+cf.COLUMNS[xname]][:Nclusters].values
...
# -------------------------- Get 1-sigma uncertainty ------------------------- #
        ...
        BestFitOutputFile = os.path.join(OutputFileDir, 'fit_all_scatter.csv')
