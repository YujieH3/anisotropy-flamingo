...
#SBATCH -J h0_dc_fit_all_scatter     #Give it something meaningful.
...
do
    ...
    if [ -f "${output}/fit-all-scatter.done" ] #use a file flag
    then
        continue
    else
        python fit-all.py -i $input -o $output -t $n -n 300 --overwrite && echo > "${output}/fit-all-scatter.done"
    fi
