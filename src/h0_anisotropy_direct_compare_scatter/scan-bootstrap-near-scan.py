...
def scan_bootstrapping(Nbootstrap : int, 
                       ...
                       B_step     : float,
                       logA_step  : float,
                       scat_obs_Y : np.ndarray,
                       scat_obs_X : np.ndarray,
                    ):
    """
    Warning
    ----
    ...
    for lon_c_rad, lat_c_rad in zip(lons_c_rad, lats_c_rad):
        ...
        cone_scat_obs_Y = scat_obs_Y[mask]
...
if __name__ == '__main__':
    ...
    for scaling_relation in relations:
        ...
        output_file = f'{output_dir}/scan_bootstrap_{scaling_relation}_theta{cone_size}_scatter.csv'
        if os.path.exists(output_file) and not overwrite:
            print(f'File exists and overwrite is set to False: {output_file}')
            continue
        
        # if best_fit_dir does not exists
        best_fit_file = f'{output_dir}/scan_best_fit_{scaling_relation}_theta{cone_size}_scatter.csv'
